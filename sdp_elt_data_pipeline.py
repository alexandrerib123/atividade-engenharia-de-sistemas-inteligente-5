...
def transform_raw_dataset(dataset_file_path: str=None) -> str:
    """
    Essa função aplica transformações no dataset bruto:
    (1) Remove arquivos irrelevantes (exemplos/documentação)
    (2) Remove linhas com maioria de métricas zeradas
    (3) Remove outliers usando o método IQR
    
    Parameters:
        dataset_file_path (str): Caminho para o dataset bruto
    
    Returns:
        transformed_file_path (str): Caminho para o dataset transformado
    """

    df = pd.read_csv(dataset_file_path, delimiter=';')
 
    exclude_paths = ['examples', 'docs', 'doc', 'tests', 'test', 'benchmarks']
    pattern = '|'.join(exclude_paths)
    df = df[~df['FILE'].str.contains(pattern, case=False, na=False)]
    
    metrics_cols = ['LOC', 'COM', 'BLK', 'NOF', 'NOC', 'APF', 'AMC', 'NER', 'NEH', 'CYC', 'MAD']
    df['zero_count'] = (df[metrics_cols] == 0).sum(axis=1)
    df = df[df['zero_count'] < len(metrics_cols) // 2]
...
def start():
    token = "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"
